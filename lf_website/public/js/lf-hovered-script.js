...
(Array.prototype.unique = function () {
	...
	document.addEventListener("DOMContentLoaded", () => {
			...
			elementExist(s) &&
				s.addEventListener("click", () => {
					__each(t).switchClass("mobi-nav-active"),
						...
						document.querySelector("#lf-logo").classList.remove("transparent");
				}),
			elementExist(r))
		) {
		...
		if (elementExist(document.querySelector(".lf-roadmap-page"))) {
			...
			p.forEach((e) => {
				e.addEventListener("click", function (e) {
					"LI" == e.target.nodeName ||
						...
						e.target.classList.contains("lf-rm-deadline") ||
						e.target.classList.contains("lf-rm-progress-report") ||
			...
			var h = document.querySelectorAll(".filter-wraps .filter-list .lf-rm-in-progress"),
				v = document.getElementById("lf-roadmap-data"),
				g = document.getElementById("lf-roadmap-template"),
				f = document.querySelector(".lf-roadmap-slider"),
				y = document.querySelector(".lf-rm-close"),
				b = document.querySelector(".lf-roadmap-slider-wrap"),
				w = new Date(),
				_ = w.getDate(),
				E = w.getMonth() + 1,
				S = w.getFullYear();
			...
			if (
				...
				var L = function () {
					f.classList.toggle("lf-slider-active"),
					...
					elementExist(t) &&
						t.forEach((e) => {
							e.classList.contains("lf-road-map-section") ||
					...
					h.forEach((e) => {
						e.addEventListener("click", function () {
							if (
								...
								(deadlineData = ""),
								(deadline = e.getElementsByClassName("lf-rm-deadline")),
								(rmContent = e.getElementsByClassName("lf-rm-content")),
								...
								data[0].data.forEach((e) => {
									(d = new Date(e.date)),
										(t += template
											...
											.replace(/{{lf_des}}/g, e.desc.replace(/\\/g, "")));
								}),
									t.length > 0 &&
										(L(),
										(bodycontent = document.querySelector(
											".lf-roadmap-slider .lf-roadmap-body"
										)),
										elementExist(bodycontent) &&
											((bodycontent.innerHTML = t),
											elementExist(
												document.querySelector(".lf-slider-deadline")
											) &&
												elementExist(
													document.querySelector(".lf-rm-head-content")
												) &&
		...
		var C = document.querySelector(".lf-extensions-page");
		if (elementExist(C)) {
			var x,
		...
		elementExist(R) &&
			(O.addEventListener("click", function () {
				__each(R).hasClass("lf-blog-active", (e, t) => {
					t
						? __each(R).removeClass("lf-blog-active")
						: __each(R).addClass("lf-blog-active");
				});
			}),
			window.addEventListener("click", function (e) {
				e.target !== O && __each(R).removeClass("lf-blog-active");
			}));
		var P = document.querySelectorAll(".lf-mp-features");
		if (elementExist(P)) {
			var H = document.querySelectorAll(".lf-mp-features .lf-mp-tab a li");
			__each(H).forAll((e) => {
				e.addEventListener("click", function () {
					__each(H).removeClass("active"), __each(this).addClass("active");
					var e = this.getAttribute("data-tab"),
						t = document.querySelectorAll(".lf-mp-features .mp-tablet-wrap");
					__each(t).removeClass("active"),
						__each(t).forAll((t) => {
		...
		if (elementExist(document.querySelector(".lf-scroll-slider"))) {
			var te = 10;
			document.querySelectorAll(".lf-scroll-slider").forEach(function (e) {
		...
		(trustpilot = document.querySelector("footer .trustpilot-widget")),
			...
			null != document.querySelector(".lf-ext-form .ext-form") &&
				((modUrl = bsXhrObj.assets_uri + "dist/js/lf-ext-form.min.js?1.3"),
				__each()
					.importModule(modUrl, "script")
					.then((e) => {}));
		var se = document.querySelector(".lf-testimonial-page");
		if (elementExist(se)) {
		...
		(bgLangModal = document.querySelector("footer #bgLangWrap")),
			...
			null != document.querySelector(".lf-social-respos-gallery") &&
				((modUrl = bsXhrObj.assets_uri + "dist/js/lf-popup-slide.min.js?1.2"),
				__each()
					.importModule(modUrl, "script")
					.then((e) => {}));
		var ce = [
		...
		if ((ne = document.querySelectorAll(".lf-video")).length) {
			let De,
				Ne,
				Oe = null;
			const Re =
			...
			const je = (e, t) => {
				...
				if (
					...
					let t = e.classList.contains("lf-shorts");
		...
		const ue = document.querySelectorAll(".lf-feedback-card");
		ue &&
			ue.forEach((e) => {
		...
		me.length &&
			me.forEach((t) => {
				t.addEventListener("click", (s) => {
					var n = t.querySelector(".lf-video");
					if (n) {
		...
		document.querySelector(".lf-hero-videos-wrapper");
		function pe(e) {
			var t = e % 60,
				s = (e - t) / 60;
			return (
		...
		document.querySelectorAll(".audio-player").forEach((e) => {
			const t = e.querySelector("audio"),
				...
				a = e.querySelector(".lf-podcast-start"),
				r = e.querySelector(".lf-podcast-end");
			a.innerHTML = "00:00";
			const o =
		...
		var he = document.getElementById("lf-star-repo-btn");
		elementExist(he) &&
			he.addEventListener("click", () => {
				...
				e.open("POST", bsXhrObj.url, !0),
					...
					e.send("action=lf_starring_bagisto");
			});
		var ve = document.querySelectorAll(".lf-star-count-alphabets .number-animate");
		if (ve) {
		...
		var ge = document.querySelector(".lf-thankyou-content span");
		if (elementExist(ge))
			var fe = ge.innerHTML,
				ye = setInterval(() => {
					(ge.innerHTML = fe--),
		...
		var be = document.querySelector(".lf-customer-showcase-wrap");
		if (void 0 !== be && null != be) {
			(q = JSON.parse(document.getElementById("bsCustomerShowcaseData").innerText)),
				...
				(k = document.getElementsByClassName("lf-customer-list-container")[0]),
				(I = document.querySelectorAll(".lf-filter-option"));
			var we = be.querySelectorAll(
					'.lf-filter-option[data-filter_group="category"] .lf-filter-label'
				),
				_e = document.querySelector(".lf-clear-filter"),
				Ee = document.querySelector("#loadMoreCustomerData"),
				Se = be.querySelectorAll(".lf-filter-option .lf-filter-label");
			bsFilterChipWrap = be.querySelector(".lf-selected-filters-chip");
			B = "all";
			const Ue = 24;
			var Le = 1;
			function Ce() {
			...
			D = function (e) {
				...
				if (t["customer-search"]) {
					...
					(s = s.filter((t) =>
						...
						document
							.querySelector(".lf-customer-search-form")
					...
					if (!t[e]) {
						...
						n.forEach((t) => {
							...
							n > 0
								...
								(t.querySelector(".lf-filter-data-count").textContent = n);
						});
					}
				};
				return (
					n(
						"category",
						document.querySelectorAll(".lf-category-filter li[data-option]")
					),
					n("region", document.querySelectorAll(".lf-region-filter li[data-option]")),
					n("useCase", document.querySelectorAll(".lf-use-case-filter li[data-option]")),
					_e.classList.toggle("display-none", !Object.keys(t).length),
					["category", "region", "useCase"].forEach((e) => {
						const s = document.querySelector(`.lf-${e}-filter`);
			...
			function N() {
				...
				const t = D(q),
					s = t.slice(0, Ue * Le);
				0 === s.length
					? (e = `<div class="lf-noresult-found text-center">\n                    <img src="${bsXhrObj.assets_uri}../images/lf-noresult-found.png"/>\n                    <p class="empty-result">No Results to Show!</p>\n                </div>`)
					: s.forEach((t) => {
					...
					document.querySelectorAll(".lf-customer-logo img").forEach((e) => {
			...
			I.forEach((e) => {
				...
				t.querySelectorAll("li").forEach((t) => {
					t.addEventListener("click", function (n) {
						...
						if (
							(I.forEach((t) => {
								if (t !== e) {
									const e = t.querySelector(".lf-filter-label"),
				...
				_e.addEventListener("click", () => {
					history.pushState({}, "", window.location.pathname),
						N(),
						qe(),
						document.querySelectorAll(".lf-filter-option li.selected").forEach((e) => {
							e.classList.remove("selected"),
								e.querySelector("input").removeAttribute("checked");
						}),
						I.forEach((e) => {
							const t = e.querySelector(".lf-filter-label"),
				...
				e.addEventListener("click", (e) => {
					((e) => {
						const t = document.querySelector("#google_translate_element select"),
							...
							a = document.querySelector(".lf-spanish-youtube"),
							r = document.querySelector(".lf-turkish-youtube"),
							o = document.getElementById("lf-main-heading");
						if (!t || !s || !n) return;
		...
		(() => {
			...
			if (e) {
				const t = document.querySelector(".gtranslate_btn"),
					...
					n = document.querySelector(".lf-spanish-youtube"),
					a = document.querySelector(".lf-turkish-youtube"),
					r = document.getElementById("lf-main-heading");
				if (
		...
		(() => {
			let e,
				t = document.querySelector(".lf-story-slider-wrapper");
			if (!elementExist(t)) return;
			let s = 0;
			const n = t.querySelectorAll(".lf-story-cover"),
				a = t.querySelector(".lf-story-content-wrapper"),
				r = t.querySelectorAll(".lf-story-nav"),
			...
			r.forEach((e, t) => {
				...
				n &&
					n.forEach((t) => {
						let s = t.querySelector(".lf-video");
						s &&
							s.addEventListener("click", () => {
								clearInterval(e), (i = !1);
							});
		...
		(() => {
			const e = document.querySelectorAll(".lf-slider-container");
			0 !== e.length &&
				e.forEach((e) => {
					const t = e.querySelector(".lf-slider"),
						s = e.querySelector(".lf-slider-controls .prev-slide"),
						n = e.querySelector(".lf-slider-controls .next-slide"),
						a = t.children.length;
					let r = 0;
					const o = () => {
			...
			(() => {
				let e = document.querySelectorAll(".lf-popover-animate");
				e.length > 0 &&
					e.forEach((e) => {
						let t = e.querySelectorAll(".lf-popover-img-wrapper");
						t.forEach((s, n) => {
			...
			(() => {
				const e = document.querySelectorAll(".lf-download-composer-command-wrapper");
				e &&
					e.forEach((e) => {
						const t = e.querySelector(".lf-copy-icon"),
...
if (
	(document.addEventListener("click", function (e) {
		if (e.target.classList.contains("highlighter-close")) {
			...
			const t = e.target.closest(".lf-highlighter-box");
			t && t.remove();
		}
	}),
	initEsSwipeSlider(),
	document.querySelector(".lf-schema-faq"))
) {
	document.querySelectorAll(".lf-schema-faq-section").forEach((e) => {
		e.addEventListener("click", function () {
			const t = e.querySelector(".lf-schema-faq-answer");
...
// ================================================================
//  新增功能：悬停展开效果 + Tabs 切换
//  注意：内容使用 kr- 前缀，与导航的 lf- 前缀区分
// ================================================================

// 悬停展开效果（Trusted By 区域）
var initPanelHover = function () {
	var panels = document.querySelectorAll(".lf-company-panel");
...
// Tabs 切换（Core Features 区域）
var initCoreFeaturesTabs = function () {
	var tabs = document.querySelectorAll(".lf-feature-tab-item");
	var previews = document.querySelectorAll(".preview-image-wrap");
	...
	tabs.forEach(function (tab) {
		tab.addEventListener("click", function () {
			...
			previews.forEach(function (preview) {
				if (preview.getAttribute("data-preview-id") === id) {
...
// ================================================================
//  扩展 DOMContentLoaded 初始化
// ================================================================

document.addEventListener("DOMContentLoaded", function () {
	// 新增：悬停展开
	if (document.querySelector(".lf-trusted-by-section")) {
		initPanelHover();
	}

	// 新增：Tabs 切换
	if (document.querySelector(".lf-core-features-section")) {
		initCoreFeaturesTabs();
	}
});
